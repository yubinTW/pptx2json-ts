...
export async function parse(file: ArrayBufferLike) {
  const slides = []
...
function processCxnSpNode(node, warpObj) {
  ...
  const idx =
    node['p:nvCxnSpPr']['p:nvPr']['p:ph'] === undefined ? undefined : node['p:nvSpPr']['p:nvPr']['p:ph']['attrs']['idx']
  const type =
...
async function processPicNode(node, warpObj) {
  ...
  // TODO: await zip.file(imgName) will be null
  ...
  for (const key in plotArea) {
    switch (key) {
      ...
      case 'c:barChart':
        chart = {
          type:
            getTextByPathList(plotArea[key], ['c:grouping', 'attrs', 'val']) === 'stacked'
              ? 'stackedBarChart'
              : 'barChart',
          data: extractChartData(plotArea[key]['c:ser'])
        }
      ...
      case 'c:areaChart':
        chart = {
          type:
            getTextByPathList(plotArea[key], ['c:grouping', 'attrs', 'val']) === 'percentStacked'
              ? 'stackedAreaChart'
              : 'areaChart',
          data: extractChartData(plotArea[key]['c:ser'])
        }
...
function getShapeFill(node, isSvgMode) {
  ...
  if (fillColor) {
    ...
    let lumMod =
      parseInt(getTextByPathList(node, ['p:spPr', 'a:solidFill', 'a:schemeClr', 'a:lumMod', 'attrs', 'val'])) / 100000
    let lumOff =
      parseInt(getTextByPathList(node, ['p:spPr', 'a:solidFill', 'a:schemeClr', 'a:lumOff', 'attrs', 'val'])) / 100000
